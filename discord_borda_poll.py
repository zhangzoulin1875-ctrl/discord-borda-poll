...
async def generate_chat_reply(message, settings: dict) -> tuple:
    ...
    system_prompt += (
        ...
        f"以及 web_search 工具查到的原始資料——這些是直接查證的"
        f"事實來源，如果跟其他資料衝突，一律以這裡為準。\n"
        f"② 【次要，需判斷】「Discord 伺服器歷史資料」搜尋結果——"
        f"這些是伺服器成員發過的真實訊息，但訊息內容可能是玩笑、惡搞、"
        f"諷刺、反串或不正確的說法，不能當作查證過的事實。\n"
        f"使用 Discord 搜尋結果時，你必須判斷訊息是否像認真陳述：\n"
        f"  - 正式公告、提案、人事任命、選舉結果等 → 可信賴\n"
        f"  - 明顯是玩笑、戲謔、誇張、反串的語氣 → 絕對不能當作事實引用，"
        f"最多只能說「有人在頻道裡開玩笑說過○○」\n"
        f"  - 只有單則訊息支持、沒有其他佐證 → 存疑，不要斷言\n"
        f"  - 如果訊息內容荒謬或不可能 → 直接忽略，不要鸚鵡學舌\n"
    ...
    if _discord_auto and "沒有找到" not in _discord_auto:
        system_prompt += (
            ...
            f"⚠️ 這些是伺服器成員發過的真實訊息，但真實不等於正確——"
            f"訊息可能是玩笑、惡搞、諷刺或不正確的說法。\n"
            f"請判斷每則訊息的語氣後再決定是否引用：\n"
            f"  - 正式公告/提案/人事任命/選舉結果 → 可直接引用\n"
            f"  - 明顯是玩笑或戲謔語氣 → 不要當作事實回答，也不要重複玩笑內容\n"
            f"  - 訊息內容荒謬或不可能為真 → 直接忽略\n"
            f"只有當訊息語氣明確是認真陳述時，才以此為依據回答，"
            f"尤其是涉及人事任命、選舉結果、提案狀態等問題——下面的資料如果是認真的公告就直接回答，不要說「不確定」或「沒有公布」。\n{_discord_auto}"
        )
        ...
        async def _run_one_tool(tc):
            ...
            try:
                if name == "search_micropedia":
                    ...
                    if result and "沒有找到" not in result:
                        result = "⚠️ 以下是 Discord 伺服器成員發過的真實訊息，但可能包含玩笑、惡搞或錯誤資訊。請判斷語氣後再決定是否引用——正式公告/任命/選舉結果可直接引用，明顯玩笑或荒謬內容請忽略。\n\n" + result
